...
def test_recover_erc20_dai(whitelist, dai_token, helpers, lido_agent):
    ...
    helpers.fund_with_dai(whitelist, dai_amount)
    receipt = whitelist.recover_erc20(dai_token, dai_amount, lido_agent, sender=lido_agent)
    assert_single_event(
        receipt,
    ...
    assert dai_token.balanceOf(whitelist) == 0, "some Dai left on the contract"
    assert (
...
def test_recover_erc20_usdt(whitelist, usdt_token, helpers, lido_agent):
    ...
    receipt = whitelist.recover_erc20(usdt_token, usdt_amount, lido_agent, sender=lido_agent)
    assert_single_event(
        receipt,
        whitelist.ERC20Recovered,
        {
    ...
    assert usdt_token.balanceOf(whitelist) == 0, "some USDT left on the contract"
    assert (
...
def test_recover_erc20_failure_due_amount(whitelist, stranger, dai_token, helpers, lido_agent):
    ...
    with reverts("Dai/insufficient-balance"):
        whitelist.recover_erc20(dai_token, dai_amount + 1, stranger, sender=lido_agent)
